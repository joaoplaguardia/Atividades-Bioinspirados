def executar_algoritmo(npop, nger, Pc, Pm, k, n_elite):
    ...
    def genetico_binario():
        ...
        for g in range(nger):
            fitness = avalia_populacao(pop)

            menor_fitness = min(fitness)
            media_fitness = np.mean(fitness)

            melhor_por_geracao.append(menor_fitness)
            media_por_geracao.append(media_fitness)
            todos_fitness.append(fitness[:])  # cópia da lista de fitness da geração

            paises = torneio(pop, fitness, k)
            pop_intermediaria = cruzamento(pop, paises, Pc)
            mutacao(pop_intermediaria, Pm)
            elitismo(pop, fitness, pop_intermediaria, n_elite)
            pop = copy.deepcopy(pop_intermediaria)

        return melhor_por_geracao, media_por_geracao, todos_fitness

    return genetico_binario()
